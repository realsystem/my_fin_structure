"""Auto-generated provider for Bank of America (AI-assisted)."""
import re
...
class BankOfAmericaProvider(StatementProvider):
    name = "Bank of America"
    bank_id = "bank_of_america"

    def can_parse(self, text: str, pages: list[str]) -> float:
        score = 0.0
        if "Customer Service Information".lower() in text.lower():
            score += 0.45
        if "BANK OF AMERICA".lower() in text.lower():
            score += 0.45
    ...
    def parse(self, text: str, pages: list[str]) -> Statement:
        ...
        # Account number
        account_number = ""
        match = re.search(r"(\d{4})\s+\d{4}\s+\d{4}\s+\d{4}", header_page)
        if match:
...
register_provider(BankOfAmericaProvider())
